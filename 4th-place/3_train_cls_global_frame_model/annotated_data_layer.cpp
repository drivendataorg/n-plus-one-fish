...
cv::Mat getMask(cv::Size const& frameSize)
{
    cv::Point center(frameSize.width/2 + (rand() % (frameSize.width/4)),
                     frameSize.height/4);
    ...
    if (rand() % 5 == 3)
    {
        ...
        center = cv::Point(frameSize.width/4 + (rand() % (frameSize.width/4)),
                           frameSize.height/4);
    }

    cv::Mat mask = cv::Mat::zeros(frameSize, CV_8UC1);

    cv::RotatedRect rrect(center, cv::Size(2000, widhtBase + (rand() % 60)), angle);
    ...
    cv::fillConvexPoly(mask, v, 4, cv::Scalar(255));
    return mask;
}
}
...
void AnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    cv::Mat img;
    if (this->data_transformer_->param_.force_color()
            || this->data_transformer_->param_.force_gray())
    {
        img = DecodeDatumToCVMat(anno_datum.datum(),
                                         this->data_transformer_->param_.force_color());
    } else
    {
        img = DecodeDatumToCVMatNative(anno_datum.datum());
    }
    if (rand() % 2)
    {
        img = rotate(img, (rand() % 2) ? 90 : 270);
    }
    if (rand() % 5 == 0)
    {
        cv::Mat mask = getMask(img.size());
        float weight = (0.1 * (2 + (rand() % 4)));
    ...
    if (rand() % 5 == 0)
    {
        cv::Mat kernel =
                (cv::Mat_<float>(3,3)
                 ...
                 0.349, 0.686, 0.168,
                 0.393, 0.769, 0.189);
        cv::Scalar irLight(240, 170, 240);
        cv::transform(img, img, kernel);
        cv::addWeighted(img, 0.4, irLight, 0.8, 1, img);
    }
    EncodeCVMatToDatum(img, "jpg", anno_datum.mutable_datum());
