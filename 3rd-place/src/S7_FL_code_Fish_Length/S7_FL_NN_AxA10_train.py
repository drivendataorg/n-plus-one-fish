...
# Parameters
EXEC_ID = ''
MODEL_ID = 'NN_AxA10'  # Model to train
STAGE = 'S7_FL'  # Stage id
...
TRAIN_ROUNDS = 6  # Total rounds to train
EPOCHS_PER_ROUND = 1  # Epochs for each training round
TOTAL_EPOCHS = TRAIN_ROUNDS * EPOCHS_PER_ROUND
TRAIN_ALL = False  # If False, train all epochs, else train until not improvement
MIN_EPOCHS = 6  # Min epochs to train

BATCH_SIZE = 16  # Batch size for training samples
SAMPLE_PER_EPOCH_MULTIPLIER = 1  # Number of times the training samples are passed per epoch 
SH_EPOCHS = [  1,    3,      5, ]  # Learning Rate Shedule: Epochs (Always set value for epoch==1)
SH_LR     = [0.1, 0.01,  0.001, ]  # Learning Rate Shedule: lr value
            ...
            if hasattr(index, '__iter__'):
                ...
                if (self.transform is not None):
                    if self.y_lst is not None:
                        rsl = [self.transform(x,y) for x,y in zip(x_rsl, y_rsl)]
                        x_rsl = [s1 for s1,s2 in rsl]
                        y_rsl = [s2 for s1,s2 in rsl]
                    else:
