        ...
        while iround < TRAIN_ROUNDS and finished_epochs < MAX_EPOCHS:
            ...
            if fold != 'ALL':
                ...
                if HOW_VALIDATE == 'in_memory':
                    ...
                    print ("TO IMPLEMENT")
                    #evals=[]
                    #for group in tqdm(create_groups_BATCH(valid_imgs_df)):
               # 
               #         imgs_batch = Parallel(n_jobs=EVAL_THREADS)(delayed(read_images_BATCH)(i) for i in group) 
               #         X_batch, Y_batch = [], []
               #         for i in imgs_batch:
               #             X_batch.append(i[0])
               #             Y_batch.append(i[1])
               ...
               #         
               #         # Evaluate
               #         for start in range(0, X_batch.shape[0], EVAL_BATCH_SIZE):
               #             end = min(start+EVAL_BATCH_SIZE, X_batch.shape[0])
               #             evaluation = Model.NNmodel.test_on_batch(X_batch[start:end], Y_batch[start:end])
               #             evals.append(np.array([end-start,] + evaluation))
               # 
               #         del(imgs_batch, X_batch, Y_batch)
               #         del(evaluation)
               #     
               #     evals = np.vstack(evals)
               #     evaluation = np.mean(evals, axis=0).tolist()[1:]
