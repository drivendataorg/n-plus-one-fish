...
CACHE_PATH_VALID = "../cache_inception_v3_train/"
if not os.path.isdir(CACHE_PATH_VALID):
    os.mkdir(CACHE_PATH_VALID)
CACHE_PATH_TEST = "../cache_inception_v3_test/"
...
def get_image_augm_dense_net(tf, bbox):
    ...
    im_big = np.zeros((AUGMENTATION_SIZE, 299, 299, 3), dtype=np.float32)

    # Orig image
    part = tf[sh0_start:sh0_end, sh1_start:sh1_end, :]
    im_big[0] = cv2.resize(part, (299, 299), cv2.INTER_LANCZOS4)
    # Mirrored image
    im_big[1] = np.fliplr(im_big[0].copy())
    part = tf[max(0, sh0_start - 15):min(tf.shape[0], sh0_end + 15), max(0, sh1_start - 15):min(tf.shape[1], sh1_end + 15), :]
    im_big[2] = cv2.resize(part, (299, 299), cv2.INTER_LANCZOS4)
    part = tf[max(0, sh0_start - 30):min(tf.shape[0], sh0_end + 30), max(0, sh1_start - 30):min(tf.shape[1], sh1_end + 30), :]
    im_big[3] = cv2.resize(part, (299, 299), cv2.INTER_LANCZOS4)
...
def create_predictions_with_inception_v3_for_validation(nfolds):
    ...
    for train_index, test_index in kfold_videos_split:
        ...
        for i in test_index:
            ...
            if not os.path.isfile(store_path):
                ...
                print('Prediction with Inception_v3 file already exists: {}'.format(store_path))


def create_predictions_with_inception_v3_for_test(nfolds):
    ...
    num_fold = 0
    videos = glob.glob(INPUT_PATH + 'test_videos/*.mp4')[::-1]
    for v in videos:
...
if __name__ == '__main__':
    num_folds = 5
    create_predictions_with_inception_v3_for_validation(num_folds)
    create_predictions_with_inception_v3_for_test(num_folds)
