...
# Parameters
EXEC_ID = ''
MODEL_ID = 'NN_AxB01'  # Model to train
...
BATCH1_SIZE = 16  # Batch size for positive samples
BATCH0_SIZE = 112  # Batch size for negative samples
...
if __name__ == '__main__':
    ...
    for fold in folds:
        ...
        train1_images = [s[0] for s in train_videos if s[1][0]==1]
        train1_targets = [s[1] for s in train_videos if s[1][0]==1]
        train0_images = [s[0] for s in train_videos if s[1][0]==0]
        train0_targets = [s[1] for s in train_videos if s[1][0]==0]
