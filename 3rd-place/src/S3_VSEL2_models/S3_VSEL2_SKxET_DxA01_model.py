...
class Model(object):
    ...
    def reset_parameters_TRAIN(self):
        ...
        self.n_estimators_per_round = 25
    ...
    def test_samples(self, samples_df):
        import matplotlib.pyplot as plt
        fpath = os.path.join(self.path_predictions, 'samples')
        if not os.path.exists(fpath):
            os.makedirs(fpath)
        for i_row in samples_df.itertuples():
            fname = "boat{}_cluster{}_{}_{}".format( i_row.boat, i_row.cluster, i_row.itype, i_row.video_id,)
            print("SAVING SAMPLE PLOT: {}".format(fname))
            pred, dset, labels = self.get_predictions(itype = i_row.itype, 
                                                       image_id = i_row.video_id, 
                                                       return_dset=True, use_cache=True)
            fig, ax = plt.subplots(figsize=[40,16])
            
            if labels is not None:
                plot_data = np.transpose(np.vstack([pred, labels]), (1,0))
            else:
                plot_data = np.transpose(np.vstack([pred,]), (1,0))
            plot_data = plot_data[0:1000,:]
