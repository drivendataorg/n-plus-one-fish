...
- This file uses DenseNet121 model for inference
...
def get_image_augm_dense_net(tf, bbox):
    ...
    im_big[1] = cv2.resize(part, (224, 224), cv2.INTER_LANCZOS4)
    part = tf[max(0, sh0_start - 50):min(tf.shape[0], sh0_end + 50), max(0, sh1_start - 30):min(tf.shape[1], sh1_end + 30), :]
...
def create_predictions_with_dense_net_for_validation(nfolds):
    ...
    for train_index, test_index in kfold_videos_split:
        ...
        for i in test_index:
            ...
            if not os.path.isfile(store_path):
                ...
                print('Prediction with DenseNet file already exists: {}'.format(store_path))


def create_predictions_with_dense_net_for_test(nfolds):
...
if __name__ == '__main__':
    num_folds = 5
    create_predictions_with_dense_net_for_validation(num_folds)
    create_predictions_with_dense_net_for_test(num_folds)
