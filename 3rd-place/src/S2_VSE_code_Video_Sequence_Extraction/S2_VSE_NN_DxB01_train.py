...
# Parameters
EXEC_ID = ''
MODEL_ID = 'NN_DxB01'  # Model to train
...
BATCH1_SIZE = 3*8*2  # Batch size for positive samples
BATCH0_SIZE = 13*8*2  # Batch size for negative samples
BATCH_SIZE = BATCH1_SIZE + BATCH0_SIZE
SAMPLE_PER_EPOCH_MULTIPLIER = 4  # Number of times the training samples are passed per epoch 
...
if __name__ == '__main__':
    ...
    mc_df, mc_reading, mc_verbose, nbc, multipl = imgs_df, True, True, num_cores, 5
    result = []
    if mc_reading:
    ...
    for fold in folds:
        ...
        train1_images = [s[0] for s in train_videos if s[1]==1]
        train1_targets = [s[1] for s in train_videos if s[1]==1]
