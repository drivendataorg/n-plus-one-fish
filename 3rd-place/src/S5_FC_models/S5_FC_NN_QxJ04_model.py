...
class Model(object):
    ...
    def reset_parameters_TRAIN(self):
        ...
        self.seed = 0
        self.train_args = {'reduce_labels':False}  # To fit CrossEntropyLoss
        self.valid_args = self.train_args.copy()
        ...
        for i_frame in frames:
            ...
            if i_bbox.iloc[0].length > 0:
                patch = Data.extract_patch_PIL(patch,(i_bbox.iloc[0].xc, i_bbox.iloc[0].yc), i_bbox.iloc[0].ang, 
                                   size=(i_bbox.iloc[0].length, int(i_bbox.iloc[0].length/1.0)))
            else:
