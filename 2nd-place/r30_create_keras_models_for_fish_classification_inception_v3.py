...
- Neural net based on Inception v3. Pretrained weights are the part of Keras module
- Can be run in parallel for 5 Folds on 5 GPUs (use FOLD_TO_CALC constant).
- You can skip this part if you already have models with name 'INCEPTION_V3_fold_*.h5' in ../models/ directory
'''
...
def get_random_boxes(batch_files, train_table):
    ...
    for i in range(len(batch_files)):
        ...
        # Add random border
        y1 += random.randint(-80, 5)
        if y1 < 0:
            y1 = 0
        if y1 > 720:
            y1 = 720

        y2 += random.randint(-5, 80)
        if y2 < 0:
            y2 = 0
        if y2 > 720:
            y2 = 720

        x1 += random.randint(-80, 5)
        if x1 < 0:
            x1 = 0
        if x1 > 1280:
            x1 = 1280

        x2 += random.randint(-5, 80)
        if x2 < 0:
            x2 = 0
        if x2 > 1280:
            x2 = 1280
    ...
    while True:
        ...
        for i in range(len(batch_files)):
            ...
            if augment:
                ...
                image = random_intensity_change(image, 25)
...
if __name__ == '__main__':
    run_cross_validation_create_models(5, 'INCEPTION_V3')
