...
class Model(object):
    ...
    def read_image(self, itype, image_id, 
        ...
        for i_frame in frames:
            ...
            if read_labels:
                label = mini_df[mini_df.frame == i_frame][self.Data.clss_none]
                if len(label) == 0:
                    labels.append(np.nan)
                else:
                    labels.append(label.values[0].astype(np.uint8))
    ...
    def get_predictions(self, itype, image_id,
        ...
        if labels is not None:
            ...
            score = log_loss(y_true, y_pred, normalize = False, labels = np.array([1,0]))
    ...
    def show_preds(self, preds, labels=None, size=(12,6), title=""):
        ...
        preds_data = preds[:,0]
        if labels is not None:
            labels_data = np.nan_to_num(np.vstack(labels)[:,0]+0.1)-0.1
