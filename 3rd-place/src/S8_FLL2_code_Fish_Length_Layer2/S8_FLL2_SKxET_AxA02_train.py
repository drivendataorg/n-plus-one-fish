...
# Parameters
EXEC_ID = ''
MODEL_ID = 'SKxET_AxA02'  # Model to train
STAGE = 'S8_FLL2'  # Stage id
...
if __name__ == '__main__':
    ...
    for fold in folds:
        ...
        x_trn = train_df[[s for s in train_df.columns if s not in \
                ['image_id', 'itype', ]+['length',]]]\
                .values.astype(np.float32)
        y_trn = train_df['length'].values.astype(np.float32)
        w_trn = train_df['sample_weight'].values.astype(np.float32)
        if fold != 'ALL':
            x_val = valid_df[[s for s in valid_df.columns if s not in \
                    ['image_id', 'itype', ]+['length',]]]\
                .values.astype(np.float32)
            y_val = valid_df['length'].values.astype(np.float32)
        ...
        for i_round in range(n_round_max):
            ...
            if fold == 'ALL':
                trn_pred = SKmodel.predict(x_trn)
                ...
                trn_pred = SKmodel.predict(x_trn)
                trn_scores = [metric(y_trn, trn_pred, sample_weight=w_trn) for metric in Model.metrics]
                val_pred = SKmodel.predict(x_val)
