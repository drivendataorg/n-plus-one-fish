...
# Parameters
EXEC_ID = ''
MODEL_ID = 'SKxET_BxA20'  # Model to train
STAGE = 'S6_FCL2'  # Stage id
...
if __name__ == '__main__':
    ...
    def parallel_function(i_row):   
        return Model.read_image(i_row.itype, i_row.image_id,  frame = 'all_train', read_targets=True) 
    ...
    for fold in folds:
        ...
        x_trn = train_df[[s for s in train_df.columns if s not in \
                ['image_id', 'itype', ]+Data.clss_names]]\
                .values.astype(np.float32)
        y_trn = train_df[Data.clss_names].values.astype(np.float32)
        w_trn = train_df['sample_weight'].values.astype(np.float32)
        if fold != 'ALL':
            x_val = valid_df[[s for s in valid_df.columns if s not in \
                    ['image_id', 'itype', ]+Data.clss_names]]\
                    .values.astype(np.float32)
            y_val = valid_df[Data.clss_names].values.astype(np.float32)
            w_val = valid_df['sample_weight'].values.astype(np.float32)
        ...
        for i_round in range(n_round_max):
            ...
            SKmodel.fit(x_trn, y_trn, w_trn)
            
            if fold == 'ALL':
                trn_pred = SKmodel.predict_proba(x_trn)
                trn_pred = np.transpose(np.array([s[:,1] for s in trn_pred]), (1,0))
                trn_scores = [metric(y_trn, trn_pred, sample_weight=w_trn) for metric in Model.metrics]
                ...
                val_pred = np.transpose(np.array([s[:,1] for s in val_pred]), (1,0))
                val_scores = [metric(y_val, val_pred, sample_weight=w_val) for metric in Model.metrics]
                print ('  Metrics: {}'.format(Model.metrics_desc))
        ...
        scores.append([fold, val_score, trn_score])
    ...
    if FOLD_ID == '' or FOLD_ID == 'COMPLETE':
        scores_pd = pd.DataFrame(scores, columns=['fold','val_score','trn_score'])  
        scores_pd.loc[len(scores_pd)] = ['MEAN', np.mean(scores_pd.val_score.values), np.mean(scores_pd.trn_score.values)]
        print scores_pd
